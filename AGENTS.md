...
| `internal/relay` | Concrete `engine.EventStore`: publishes/fetches file-entry events over go-nostr websockets to one or more relays. Persistent, lazily-reconnecting connections; publish to all (succeed if any accepts), fetch unions + dedupes by event ID. Tested against a minimal in-process NIP-01 relay (`coder/websocket`). |
...
Not yet built (next milestones): config **discovery** (NIP-65/kind-10063 published lists — until then `daemon` requires `--relay`/`--blossom` set at enroll), fsnotify watch (with debounce/settle, to complement the periodic reconcile), retry/backoff policy per docs (the loop currently logs a failed pass and retries on the fixed interval), Blossom multi-server mirroring (daemon uses the first server), tombstone re-assertion (the relay may drop a tombstone under a newer concurrent edit — see the known-limitation note in `nostrevent`), and the tray.
