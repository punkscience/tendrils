...
| `cmd/tendrils` | **cobra** CLI: `keygen`, `enroll` (`--key`/`--root`, reuses stored key), `status` (pending/conflicts from scan-vs-index), `daemon` (validates enrollment; engine stubbed). |

Not yet built (next milestones): `internal/engine` (orchestrates watch→settle→encrypt→upload→publish + periodic reconcile, and re-asserts tombstones the relay may drop — see the known-limitation note in `nostrevent`), the relay client, config **discovery** (NIP-65/kind-10063 published lists), fsnotify watch, and the tray.

**Design gap surfaced by `internal/blob`, for the engine to resolve:** a Blossom blob is addressed by the SHA-256 of its *stored* bytes, i.e. the encrypted form. But `tree.Entry.Sha256` is the *plaintext* hash (file identity + dedup), and encryption uses a random nonce per blob, so the same file seals to different bytes — hence a different content address — on every device. The event a device publishes must therefore carry the sealed-blob address it actually uploaded, separate from the plaintext hash; a puller fetches by that address, then unseals. `nostrevent`/`tree.Entry` currently have no field for this second hash. (The `tree.Entry.Sha256` doc comment's claim that the plaintext hash is "the blob's content address on the Blossom server" is therefore aspirational, not literal.)
