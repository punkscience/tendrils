...
| `cmd/tendrils` | **cobra** CLI: `keygen`, `enroll` (`--key`/`--root`, reuses stored key), `status` (pending/conflicts from scan-vs-index), `daemon` (validates enrollment; engine not yet wired to a live relay). |

Not yet built (next milestones): the concrete **relay client** (a go-nostr websocket adapter satisfying `engine.EventStore`), wiring `daemon` to build an `Engine` from config and run a loop (periodic reconcile + retry/backoff per docs), config **discovery** (NIP-65/kind-10063 published lists), fsnotify watch (with debounce/settle), tombstone re-assertion (the relay may drop a tombstone under a newer concurrent edit — see the known-limitation note in `nostrevent`), and the tray.
